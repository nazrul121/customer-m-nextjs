...
import { ExpenseHeadSchema } from '@/lib/schemas'
...
export async function GET(request: Request) {
  try {
    ...
    if (isSimple) {
      const list = await prisma.expenseHead.findMany({
    ...
    const [expenseHeads, totalCount] = await prisma.$transaction([
      prisma.expenseHead.findMany({
        ...
        include: {
          _count: {
            select: { expenses: true } // This counts how many services use this type
          }
        }
      }),
      // B) Count the total number of records matching the search filter
      prisma.expenseHead.count({ where: whereClause }),
    ]);
    ...
    return NextResponse.json({ message: 'Failed to fetch expense head' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    // 1. Validate with Zod
    const validationResult = ExpenseHeadSchema.safeParse(data);
    ...
    const existingType = await prisma.expenseHead.findFirst({
        where: {
          OR: [ { title: title } ]
        }
    });
    ...
    const newRow = await prisma.expenseHead.create({
...
export async function PUT(request: Request) {
  try {
    ...
    const updateData = ExpenseHeadSchema.partial().safeParse({ title, description })
    ...
    // Perform update
    const updatedType = await prisma.expenseHead.update({
      where: { id: String(id) },
      data: validatedData,
    })
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
        return NextResponse.json({ message: 'Expense Head not found.' }, { status: 404 });
    }
    console.error('Error updating Expense Head:', error)
    return NextResponse.json({ message: 'Failed to update Expense Head' }, { status: 500 })
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    const row = await prisma.expenseHead.findUnique({ where: { id } });
    if (!row) {
